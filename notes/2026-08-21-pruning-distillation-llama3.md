...
<p class='lede'>Three ideas from one page of a notebook: what makes a tokenizer good, what it means to cut a model down, and how the cut-down model is taught to behave like the one it came from. They belong together because they are the three places a model's size actually gets decided — how much text a token carries, how many weights survive, and how well the survivors are retrained.</p>
...
<p class='cap'>Each bar is how much <b>original text</b> a context window holds at the current ratio; the dashed line is your prompt. The lab uses the exact windows — 8,192 / 32,768 / 131,072 tokens — so at 0.75 it reads 6,144 / 24,576 / 98,304 words, a little above the notebook's rounded 6,000 / 24,000 / 96,000.</p>
</div>
</div>
...
<p>Distillation on “logits” means the second kind. When the notebook diagram labels the student's and teacher's outputs <i>(vocabulary logits)</i>, that is the disambiguation.</p>
</div>
...
One honest wrinkle. Written as $\mathrm{KL}(P\|Q)$ with $P$ the student and $Q$ the teacher, that is the *reverse* KL — it punishes the student for putting probability mass where the teacher put none, and lets it quietly drop modes the teacher had. Minitron minimises the **forward** KL, with the teacher as $P$, and drops the ground-truth cross-entropy term entirely. The lab above has a toggle for the direction; the two values differ, sometimes a lot, and they only agree once the student has actually converged onto the teacher.
...
<div class='caption'><span class='caption-label'>Figure 5.</span> The five steps, in the order the notebook draws them: train the teacher, estimate importance, rank, trim, distil. Ranking and trimming are one operation split in two — the ranking decides everything, the trim just applies it.</div>
</div>
