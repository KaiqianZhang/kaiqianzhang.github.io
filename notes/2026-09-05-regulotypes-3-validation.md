...
Here $\widetilde Y_{is}$ is the scaled molecular phenotype for measurement $i$ at reference pair $s$, and $\widetilde X_{is}$ the corresponding scaled donor genotype. The term $c_i^\mathsf{T}a_s$ is the additive covariate contribution and $b_{d(i)s}$ the donor-specific random intercept, both of which exist to absorb variation that has nothing to do with genotype. Everything the method is actually about sits inside the bracket: $\beta_s$ is the average cis effect of the pair, $u_i$ is the latent cellular context of measurement $i$, and $\lambda_s$ says how strongly that particular pair responds to it.
...
Both programs need the same reference variant–gene pairs, the same $\widetilde Y$ and $\widetilde X$, the same covariate matrix $C$ and donor index $d(i)$, the same $K$, the same Gaussian priors and hyperparameters, and the same treatment of the donor random effect. Where it can be arranged they should also share an initialization, an update order and a stopping rule — those three are not part of the mathematical problem, but they are part of the answer a variational algorithm returns, and leaving them free turns an exact comparison into an approximate one.
...
so **$r^\star_{si}=\hat r_{si}$**: the shift the intercept absorbed is exactly the shift the coordinate lost. The same argument applies factor by factor when $K>1$, which is worth checking rather than assuming, since a loop over $k$ is where an implementation is most likely to compensate a shift with the wrong factor's mean.
...
The primary assertion is **$\Delta_R\approx0$**, and I would test both
...
Therefore the effect measured per unit of the original genotype is **$r_{si} = (q_s/h_s)\,\widetilde r_{si}$** — the ratio of the two scales, and nothing more.
...
Therefore **$K=1$ with $\lambda_s\equiv0$ is the $K=0$ model**, not an approximation to it.
...
The target is **$\hat R^{\mathrm{projection}}_{s:} \approx \hat R^{\mathrm{reference}}_{s:}$**.
...
A single well-conditioned setting will pass even with a badly wrong implementation, because near the middle of the parameter space almost everything agrees with almost everything. I would sweep a grid instead: $\hat a_j$ near zero and $\hat a_j$ far out past $s_j$, small and large $s_j$, small and large $\omega$, $\pi_j$ at $0.01$, $0.5$ and $0.99$, and both signs of $\hat a_j$. The settings are not the point, though. What I am covering is the *posterior*, and between them those choices produce one that is

* almost entirely the point mass at zero;
* genuinely undecided between zero and nonzero;
* almost certainly nonzero;
* strongly shrunk;
* barely shrunk at all.

Those five are where two implementations part company, and the middle one most of all, because that is where the weight $w_j$ is doing real work rather than saturating. For every case, compare
...
Three of them are reductions to something already trusted. The Gaussian fit reducing to the released SURGE implementation covers the basic likelihood and the variational machinery, and it is the only link on this page that reaches outside my own code. The point-normal prior reducing to the Gaussian update at $\pi=1$ covers the new prior machinery, and it earns its place mostly through where it attaches: it hands the empirical-Bayes half of the method back to the half SURGE has already checked. The third, $K=1$ with $\lambda=0$ collapsing to $K=0$, stays inside my own code but is exact, and it says the interaction component disappears precisely when it should rather than merely becoming small.

Three are invariances, and each one guards a different kind of arbitrariness. Donor-balanced reparameterization moves $(\beta,\Lambda,U)$ a long way and has to leave $R$ exactly where it was, which is what makes the coordinates safe to plot. Unscaling carries an effect from the fitting scale back into phenotype units and has to reproduce the same contribution to the phenotype by either route, which is what makes the numbers safe to report. Self-projection re-solves a pair the fit already knows and has to return the profile the fit already assigned it — the same machinery that will later carry held-out genes onto the map, tested in the one place where the answer happens to be available.

The seventh belongs to neither group. Comparing the analytic cEBNM posterior against a numerical integration of the same posterior is the only check here where a second, genuinely independent computation of the same quantity exists. Every other test on this page would still pass if I had misread the specification and implemented my misreading consistently. That one would not.
...
That sentence covers a great deal and promises very little. The baseline likelihood agrees with an implementation somebody else wrote; the reparameterization leaves $R$ alone; effects come back in the right phenotype units; the interaction term empties exactly when it should; the projection solves the conditional problem it claims to; the empirical-Bayes moments are what two unrelated computations both say they are; and the new prior collapses onto the old one where the two are supposed to meet.

None of that asks whether the estimated regulotype is scientifically meaningful, and it is worth being blunt about how much room that leaves. A perfectly coded algorithm can still fail to recover a weak signal. It can estimate an unstable map when there are too few donors, struggle when there are too few independent reference loci, and recover the wrong cellular geometry when the statistical problem is poorly identified. None of those would register anywhere on this page. Every check here would still pass, and pass at machine precision.
